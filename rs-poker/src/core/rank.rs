...
fn rank_straight(value_set: u32) -> Option<u32> {
  ...
  //       -------------
  //       0000000000000
  let left = value_set & (value_set << 1) & (value_set << 2) & (value_set << 3) & (value_set << 4);
...
mod tests {
  ...
  fn test_two_pair() {
    ...
    let rank = (1 << Value::Ace as u32 | 1 << Value::Nine as u32) << 13 | 1 << Value::Ten as u32;
  ...
  fn test_rank_seven_straights() {
    ...
    for (idx, s) in straights.iter().enumerate() {
      assert_eq!(Rank::Straight(idx as u32 + 1), Hand::new_from_str(s).unwrap().rank());
    }
  }
