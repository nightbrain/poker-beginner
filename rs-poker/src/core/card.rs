...
mod tests {
  ...
  #[test]
  fn test_compare() {
    let c1 = Card { value: Value::Three, suit: Suit::Spade };
    let c2 = Card { value: Value::Four, suit: Suit::Spade };
    let c3 = Card { value: Value::Four, suit: Suit::Club };
  ...
  fn test_gap() {
    ...
    assert_eq!(0, Value::Two.gap(Value::Two));
    // Test one gap at the low end
    assert_eq!(1, Value::Two.gap(Value::Three));
    // test that ordering doesn't matter
    assert_eq!(1, Value::Three.gap(Value::Two));
