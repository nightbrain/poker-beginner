...
impl<'a> CardIter<'a> {
  ...
  pub fn new(possible_cards: &[Card], num_cards: usize) -> CardIter {
    ...
    CardIter { possible_cards, idx, num_cards }
  }
}
...
impl<'a> Iterator for CardIter<'a> {
  ...
  fn next(&mut self) -> Option<Vec<Card>> {
    ...
    let result_cards: Vec<Card> =
      self.idx.iter().map(|i| self.possible_cards[*i as usize]).collect();
    Some(result_cards)
  }
}
...
mod tests {
  ...
  fn test_iter_one() {
    ...
    h.push(Card { value: Value::Two, suit: Suit::Spade });
  ...
  fn test_iter_two() {
    let mut h = Hand::default();
    h.push(Card { value: Value::Two, suit: Suit::Spade });
    h.push(Card { value: Value::Three, suit: Suit::Spade });
    h.push(Card { value: Value::Four, suit: Suit::Spade });
