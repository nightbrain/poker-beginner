...
impl RangeIter {
  ...
  fn first_card(&self) -> Card {
    ...
    // Create the card.
    Card { value: v, suit: Suit::from_u8(self.suit_one_offset) }
  }
...
impl RangeParser {
  ...
  ///
  /// for hand in hands {
  ///     assert_eq!(hand[0].value, hand[1].value);
  ...
  pub fn parse_one(r_str: &str) -> Result<Vec<Hand>, String> {
    ...
    let mut first_range = InclusiveValueRange { start: Value::Two, end: Value::Ace };
    let mut second_range = InclusiveValueRange { start: Value::Two, end: Value::Ace };
    ...
    // Get the first char.
    let fv_char =
      iter.next().ok_or_else(|| String::from("Error getting the first card of the hand"))?;
    ...
    let sv_char =
      iter.next().ok_or_else(|| String::from("Error getting the second card of the hand."))?;
    ...
    while let Some(m) = Modifier::from_char(*iter.peek().unwrap_or(&':')) {
      ...
      match m {
        ...
        Modifier::Range => {
          let fr_char = iter
            .next()
            .ok_or_else(|| String::from("Error getting the first card of the end of the range"))?;
          let sr_char = iter
            .next()
            .ok_or_else(|| String::from("Error getting the second card of the end of the range"))?;
          first_range.end =
            Value::from_char(fr_char).ok_or_else(|| String::from("Error parsing the range"))?;
          second_range.end =
            Value::from_char(sr_char).ok_or_else(|| String::from("Error parsing the range"))?;
...
mod test {
  ...
  fn test_range_iter_static() {
    let c =
      RangeIter::stat(Value::Ace, InclusiveValueRange { start: Value::Two, end: Value::King });
  ...
  fn test_easy_parse_sorted() {
    ...
    assert_eq!(RangeParser::parse_one("AK").unwrap(), RangeParser::parse_one("KA").unwrap());
  }
  ...
  fn test_explicit_pair_good() {
    assert!(!RangeParser::parse_one(&String::from("2c2s")).unwrap().is_empty());
  }
  #[test]
  fn test_explicit_suit_good() {
    assert!(!RangeParser::parse_one(&String::from("6c2c")).unwrap().is_empty());
  }
  #[test]
  ...
  fn test_explicit_suit_plus() {
    assert!(!RangeParser::parse_one(&String::from("2s2+")).unwrap().is_empty());
  }
  #[test]
  fn test_explicit_suit_pair() {
    assert!(!RangeParser::parse_one(&String::from("8D8")).unwrap().is_empty());
  }
  #[test]
