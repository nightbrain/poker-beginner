    ...
    Suit::suits()
      ...
      .map(|s| {
        Hand::new_with_cards(vec![
          Card { value: self.value_one, suit: *s },
          Card { value: self.value_two, suit: *s },
        ])
      })
      .collect()
  }
    ...
    for (i, suit_one) in suits.iter().enumerate() {
      for suit_two in &suits[i + 1..] {
        ...
        hands.push(Hand::new_with_cards(vec![
          Card { value: self.value_one, suit: *suit_one },
          Card { value: self.value_two, suit: *suit_two },
        ]));

        // If this isn't a pair then the flipped suits is needed.
        if self.value_one != self.value_two {
          hands.push(Hand::new_with_cards(vec![
            Card { value: self.value_one, suit: *suit_two },
            Card { value: self.value_two, suit: *suit_one },
          ]));
        }
      }
    }
...
impl SingleCardRange {
  ...
  fn possible_hands(&self) -> Vec<Hand> {
    ...
    while cur_value <= self.end {
      let mut new_hands =
        Default { value_one: self.value_one, value_two: cur_value, suited: self.suited }
          .possible_hands();
...
impl StartingHand {
  ...
  pub fn default(value_one: Value, value_two: Value, suited: Suitedness) -> Self {
    Self::Def(Default { value_one, value_two, suited })
  }
  ...
  pub fn single_range(value_one: Value, start: Value, end: Value, suited: Suitedness) -> Self {
    Self::SingleCardRange(SingleCardRange { value_one, start, end, suited })
  }
...
mod tests {
  ...
  #[test]
  fn test_aces() {
    let sh = Default { value_one: Value::Ace, value_two: Value::Ace, suited: Suitedness::OffSuit };
    assert_eq!(6, sh.possible_hands().len());
  }

  #[test]
  fn test_suited_connector() {
    let sh = Default { value_one: Value::Ace, value_two: Value::King, suited: Suitedness::Suited };
    assert_eq!(4, sh.possible_hands().len());
  }
  #[test]
  fn test_unsuited_connector() {
    let sh = Default { value_one: Value::Ace, value_two: Value::King, suited: Suitedness::OffSuit };
    assert_eq!(12, sh.possible_hands().len());
  }

  #[test]
  fn test_starting_hand_count() {
    let num_to_test: usize = StartingHand::all().iter().map(|h| h.possible_hands().len()).sum();
    assert_eq!(1326, num_to_test);
  }
}
